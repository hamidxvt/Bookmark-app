...
const features = [
  { icon: Navigation, title: "Live GPS Tracking", body: "Monitor every field officer in real time" },
  { icon: Activity, title: "Visit Intelligence", body: "Track routes, visits and performance" },
  { icon: Users, title: "Customer Network", body: "Manage thousands of schools, shops and retailers" },
    ...
    <div className="grid min-h-screen lg:grid-cols-[3fr_2fr]">
      ...
      <div className="relative hidden overflow-hidden p-12 text-navy-foreground lg:flex lg:flex-col lg:justify-between xl:p-16">
        ...
        <div
          ...
          style={{ background: "radial-gradient(circle, oklch(0.472 0.183 15.5 / 0.42), transparent 66%)" }}
        />
        <div
          className="absolute -bottom-48 -left-24 h-[30rem] w-[30rem] rounded-full"
          style={{ background: "radial-gradient(circle, oklch(0.472 0.183 15.5 / 0.22), transparent 68%)" }}
        />

        <div className="relative flex items-center gap-4">
          <div className="flex h-16 w-16 items-center justify-center rounded-3xl bg-white shadow-brand">
            <Image src="/bookmark-logo.png" alt="Bookmark" width={44} height={44} priority className="h-11 w-11 object-contain" />
          </div>
          <div>
        ...
        <div className="relative mt-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
          ...
          <p className="mt-5 max-w-xl text-sm leading-relaxed text-navy-foreground/70">
            Track officers, monitor visits, manage customers and optimize field performance in real time.
          </p>
          ...
          <div className="relative mt-10 rounded-3xl border border-white/10 bg-white/[0.04] p-4 shadow-elevated backdrop-blur-xl">
            ...
            <div className="relative mt-3 h-52 overflow-hidden rounded-2xl border border-white/10">
              <div className="map-grid relative h-full w-full opacity-90">
                {pins.map((p) => (
                  <span key={p.x + p.y} className="float-pin absolute" style={{ left: p.x, top: p.y, animationDelay: p.d }}>
              ...
              <div className="absolute bottom-3 left-3 rounded-xl border border-white/10 bg-navy/70 px-3 py-2 backdrop-blur">
                ...
                <p className="text-xs font-medium text-navy-foreground/85">Karachi · Lahore · Islamabad · Multan</p>
              </div>
            </div>
          </div>
            ...
            {features.map((f) => (
              ...
              >
                ...
                <span className="min-w-0">
                  ...
                  <span className="block text-xs leading-relaxed text-navy-foreground/60">{f.body}</span>
                </span>
              </div>
            ))}
          </div>
        ...
        <p className="relative mt-12 flex items-center gap-2 text-xs text-navy-foreground/50">
          <ShieldCheck className="h-4 w-4" /> Enterprise-grade security · Activity logged
        </p>
      </div>
        ...
        <div className="w-full max-w-lg">
          <div className="mb-8 flex items-center gap-3 lg:hidden">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-card shadow-card">
              <Image src="/bookmark-logo.png" alt="Bookmark" width={28} height={28} priority className="h-7 w-7 object-contain" />
            </div>
            <div>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 rounded-3xl bg-card p-10 shadow-elevated duration-500">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Sign in to access your field operations dashboard.</p>

            {error && (
              <div className="mt-6 flex items-start gap-2.5 rounded-xl border border-destructive/20 bg-destructive/10 px-4 py-3 text-sm text-destructive">
                <span>{error}</span>
              </div>
            )}
                ...
                <button
                  ...
                  onClick={() =>
                    toast.info("Contact your administrator", {
                      description: "Password resets are handled by the Bookmark admin team.",
                    })
                  }
          ...
          <div className="mt-8 text-center">
            <p className="text-xs font-semibold tracking-[0.14em] text-muted-foreground">SECURE ENTERPRISE ACCESS</p>
