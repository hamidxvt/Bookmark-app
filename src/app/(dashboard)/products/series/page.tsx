  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-slate-900">Series</h1>
        <button className="rounded-xl bg-[#0a1628] px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800 transition-colors cursor-pointer">+ Add Series</button>
      </div>
          ...
          <tbody className="divide-y divide-slate-50">
            {SERIES.map((s, i) => (
              <tr key={s.id} className="group hover:bg-slate-50/70 transition-colors">
                ...
                <td className="px-6 py-4"><span className="inline-flex rounded-full bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-700">{s.products} products</span></td>
