...
const POLICY_TOGGLES = [
  { key: "requireGps", label: "Require GPS to start shift", def: true },
  { key: "autoMissed", label: "Auto-mark missed visits at midnight", def: true },
  { key: "allowAdhoc", label: "Allow ad-hoc visits without approval", def: false },
];
...
export default function SettingsClient() {
  ...
  useEffect(() => {
    setHasEnvKey(Boolean(process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY));
    ...
    <div className="space-y-6 px-6 py-6 lg:px-8">
      <SectionCard title="Field Policy" description="Applies to every officer in the field">
        ...
        <ul className="mt-5 space-y-4">
          {POLICY_TOGGLES.map(({ key, label, def }) => (
            <li key={key} className="flex items-center justify-between gap-4 rounded-xl border border-border/70 p-4">
              <p className="flex items-center gap-2 text-sm font-medium text-foreground">
                <SlidersHorizontal className="h-4 w-4 text-primary" /> {label}
              </p>
              <Switch defaultChecked={def} onCheckedChange={(v) => toast.success(`${label} ${v ? "enabled" : "disabled"}`)} />
            </li>
          ))}
        </ul>
      ...
      <SectionCard title="Account" description="Personal information, security and notifications">
        <Button asChild variant="outline" className="rounded-xl">
          <Link href="/profile">
            <UserRound className="h-4 w-4" /> Open my profile
          </Link>
        </Button>
      </SectionCard>
    </div>
