...
public abstract class AbstractFuture<V> extends InternalFutureFailureAccess implements ListenableFuture<V> {
    ...
    static {
        ...
        AtomicHelper helper;
        try {
            generateCancellationCauses = Boolean.parseBoolean(System.getProperty("guava.concurrent.generate_cancellation_cause", BooleanUtils.FALSE));
    ...
    public V get(long timeout, TimeUnit unit) throws InterruptedException, TimeoutException, ExecutionException {
        ...
        String message = new StringBuilder(String.valueOf(lowerCase).length() + 28).append("Waited ").append(timeout).append(StringUtils.SPACE).append(lowerCase).toString();
        if (remainingNanos + 1000 < 0) {
            ...
            if (overWaitUnits > 0) {
                ...
                String message3 = new StringBuilder(String.valueOf(valueOf).length() + 21 + String.valueOf(unitString).length()).append(valueOf).append(overWaitUnits).append(StringUtils.SPACE).append(unitString).toString();
                if (shouldShowExtraNanos) {
                    message3 = String.valueOf(message3).concat(",");
                }
                message2 = String.valueOf(message3).concat(StringUtils.SPACE);
            }
            if (shouldShowExtraNanos) {
