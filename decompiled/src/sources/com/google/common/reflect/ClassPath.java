...
public final class ClassPath {
    ...
    public ImmutableSet<ClassInfo> getTopLevelClassesRecursive(String packageName) {
        Preconditions.checkNotNull(packageName);
        String packagePrefix = new StringBuilder(String.valueOf(packageName).length() + 1).append(packageName).append(ClassUtils.PACKAGE_SEPARATOR_CHAR).toString();
        ImmutableSet.Builder<ClassInfo> builder = ImmutableSet.builder();
        UnmodifiableIterator<ClassInfo> it = getTopLevelClasses().iterator();
        while (it.hasNext()) {
            ClassInfo classInfo = it.next();
    ...
    static String getClassName(String filename) {
        ...
        return filename.substring(0, classNameEnd).replace('/', ClassUtils.PACKAGE_SEPARATOR_CHAR);
    }
