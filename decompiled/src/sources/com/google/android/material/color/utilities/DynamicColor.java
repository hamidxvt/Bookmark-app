...
public final class DynamicColor {
    ...
    public double getTone(DynamicScheme scheme) {
        ...
        double fTone;
        double fInitialTone;
        boolean decreasingContrast = scheme.contrastLevel < Utils.DOUBLE_EPSILON;
        if (this.toneDeltaPair == null) {
            ...
            if (this.secondBackground != null) {
                ...
                return darkOption == -1.0d ? Utils.DOUBLE_EPSILON : darkOption;
            }
            return answer;
        }
        ...
        if ((fTone - nTone) * expansionDir < delta) {
            double fTone2 = MathUtils.clampDouble(Utils.DOUBLE_EPSILON, 100.0d, nTone + (delta * expansionDir));
            if ((fTone2 - nTone) * expansionDir < delta) {
                nTone = MathUtils.clampDouble(Utils.DOUBLE_EPSILON, 100.0d, fTone2 - (delta * expansionDir));
                fInitialTone = fTone2;
            } else {
                fInitialTone = fTone2;
            }
        ...
        if (50.0d > nTone || nTone >= 60.0d) {
            if (50.0d <= fInitialTone && fInitialTone < 60.0d) {
                if (stayTogether) {
                    if (expansionDir > Utils.DOUBLE_EPSILON) {
                        nTone = 60.0d;
                ...
                } else if (expansionDir > Utils.DOUBLE_EPSILON) {
                    fInitialTone = 60.0d;
                } else {
                    fInitialTone = 49.0d;
                }
