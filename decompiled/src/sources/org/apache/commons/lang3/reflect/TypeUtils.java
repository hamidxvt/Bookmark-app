...
public class TypeUtils {
    ...
    private static void appendRecursiveTypes(StringBuilder builder, int[] recursiveTypeIndexes, Type[] argumentTypes) {
        for (int i = 0; i < recursiveTypeIndexes.length; i++) {
            appendAllTo(builder.append(Typography.less), ", ", argumentTypes[i].toString()).append(Typography.greater);
        }
        Type[] argumentsFiltered = (Type[]) ArrayUtils.removeAll((Object[]) argumentTypes, recursiveTypeIndexes);
        if (argumentsFiltered.length > 0) {
            appendAllTo(builder.append(Typography.less), ", ", argumentsFiltered).append(Typography.greater);
        }
    }
    ...
    private static String classToString(Class<?> cls) {
        ...
        if (cls.getEnclosingClass() != null) {
            buf.append(classToString(cls.getEnclosingClass())).append(ClassUtils.PACKAGE_SEPARATOR_CHAR).append(cls.getSimpleName());
        } else {
    ...
    private static Map<TypeVariable<?>, Type> getTypeArguments(ParameterizedType parameterizedType, Class<?> cls, Map<TypeVariable<?>, Type> map) {
        ...
        return getTypeArguments(getClosestParentType(rawType, cls), cls, (Map<TypeVariable<?>, Type>) map2);
    }
    ...
    private static String parameterizedTypeToString(ParameterizedType parameterizedType) {
        ...
        if (useOwner == null) {
            ...
            builder.append(ClassUtils.PACKAGE_SEPARATOR_CHAR).append(raw.getSimpleName());
        }
        ...
        if (recursiveTypeIndexes.length > 0) {
            ...
            appendAllTo(builder.append(Typography.less), ", ", parameterizedType.getActualTypeArguments()).append(Typography.greater);
        }
