...
public class JobSupport implements Job, ChildJob, ParentJob {
    ...
    */
    ...
    public final ChildHandle attachChild(ChildJob child) {
                ...
                if (list$iv == null) {
                    ...
                    if (!addedBeforeCancellation2) {
                        ...
                        if (latestState instanceof Finishing) {
                            th = ((Finishing) latestState).getRootCause();
                        } else {
                            ...
                            th = completedExceptionally != null ? completedExceptionally.cause : null;
                        }
                        Throwable rootCause = th;
                        node.invoke(rootCause);
                        if (addedBeforeCompletion) {
                            if (DebugKt.getASSERTIONS_ENABLED()) {
                                if ((rootCause != null ? 1 : 0) == 0) {
                                    throw new AssertionError();
                                }
                            }
                        } else {
        ...
        boolean added = addedBeforeCancellation;
        if (added) {
            return node;
        }
        Object state$kotlinx_coroutines_core = getState$kotlinx_coroutines_core();
