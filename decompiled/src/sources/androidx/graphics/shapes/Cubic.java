...
public class Cubic {
    ...
    private Cubic(long anchor0, long control0, long control1, long anchor1) {
        this(new float[]{PointKt.m116getXDnnuFBc(anchor0), PointKt.m117getYDnnuFBc(anchor0), PointKt.m116getXDnnuFBc(control0), PointKt.m117getYDnnuFBc(control0), PointKt.m116getXDnnuFBc(control1), PointKt.m117getYDnnuFBc(control1), PointKt.m116getXDnnuFBc(anchor1), PointKt.m117getYDnnuFBc(anchor1)});
    }

    /* renamed from: pointOnCurve-OOQOV4g$graphics_shapes_release, reason: not valid java name */
    public final long m104pointOnCurveOOQOV4g$graphics_shapes_release(float t) {
        float u = 1 - t;
        float f = 3;
        return FloatFloatPair.m12constructorimpl((getAnchor0X() * u * u * u) + (getControl0X() * f * t * u * u) + (getControl1X() * f * t * t * u) + (getAnchor1X() * t * t * t), (getAnchor0Y() * u * u * u) + (getControl0Y() * f * t * u * u) + (getControl1Y() * f * t * t * u) + (getAnchor1Y() * t * t * t));
    }
    ...
    public final void calculateBounds$graphics_shapes_release(float[] bounds, boolean approximate) {
        ...
        float minY5 = Math.min(getAnchor0Y(), getAnchor1Y());
        ...
        if (approximate) {
            ...
            bounds[1] = Math.min(minY5, Math.min(getControl0Y(), getControl1Y()));
        ...
        if (zeroIsh(xa)) {
            if (xb == 0.0f) {
                ...
                if (!(0.0f <= t && t <= 1.0f)) {
                    ...
                    float it = PointKt.m116getXDnnuFBc(m104pointOnCurveOOQOV4g$graphics_shapes_release(t));
                    if (it < minX) {
                        minX = it;
                    }
                    if (it > maxX) {
            ...
            if (xs < 0.0f) {
                ...
                if (0.0f <= t1 && t1 <= 1.0f) {
                    float it2 = PointKt.m116getXDnnuFBc(m104pointOnCurveOOQOV4g$graphics_shapes_release(t1));
                    if (it2 < minX) {
                        minX = it2;
                    }
                    if (it2 > maxX) {
                ...
                if (0.0f <= t2 && t2 <= 1.0f) {
                    float it3 = PointKt.m116getXDnnuFBc(m104pointOnCurveOOQOV4g$graphics_shapes_release(t2));
                    if (it3 < minX) {
                        minX = it3;
                    }
                    if (it3 > maxX) {
        ...
        float yb = ((getAnchor0Y() * f2) - (minY2 * getControl0Y())) + (getControl1Y() * f2);
        float yc = (-getAnchor0Y()) + getControl0Y();
        if (!zeroIsh(ya)) {
            float ys = (yb * yb) - ((minY2 * ya) * yc);
            if (ys >= 0.0f) {
                float t12 = ((-yb) + ((float) Math.sqrt(ys))) / (f2 * ya);
                if (!(0.0f <= t12 && t12 <= 1.0f)) {
                    minY3 = minY;
                } else {
                    float it4 = PointKt.m117getYDnnuFBc(m104pointOnCurveOOQOV4g$graphics_shapes_release(t12));
                ...
                if (!(0.0f <= t22 && t22 <= 1.0f)) {
                    minY4 = minY3;
                } else {
                    float it5 = PointKt.m117getYDnnuFBc(m104pointOnCurveOOQOV4g$graphics_shapes_release(t22));
                    if (it5 < minY3) {
                        minY3 = it5;
                    }
                    if (it5 > maxY) {
            ...
            if (!(yb == 0.0f)) {
                float t3 = (f2 * yc) / ((-2) * yb);
                if (0.0f <= t3 && t3 <= 1.0f) {
                    float it6 = PointKt.m117getYDnnuFBc(m104pointOnCurveOOQOV4g$graphics_shapes_release(t3));
                    if (it6 < minY) {
                        minY = it6;
                    }
                    if (it6 > maxY) {
    ...
    public final Pair<Cubic, Cubic> split(float t) {
        float u = 1 - t;
        long pointOnCurve = m104pointOnCurveOOQOV4g$graphics_shapes_release(t);
        float f = 2;
        return TuplesKt.to(CubicKt.Cubic(getAnchor0X(), getAnchor0Y(), (getAnchor0X() * u) + (getControl0X() * t), (getAnchor0Y() * u) + (getControl0Y() * t), (getAnchor0X() * u * u) + (getControl0X() * f * u * t) + (getControl1X() * t * t), (getAnchor0Y() * u * u) + (getControl0Y() * f * u * t) + (getControl1Y() * t * t), PointKt.m116getXDnnuFBc(pointOnCurve), PointKt.m117getYDnnuFBc(pointOnCurve)), CubicKt.Cubic(PointKt.m116getXDnnuFBc(pointOnCurve), PointKt.m117getYDnnuFBc(pointOnCurve), (getControl0X() * u * u) + (getControl1X() * f * u * t) + (getAnchor1X() * t * t), (getControl0Y() * u * u) + (getControl1Y() * f * u * t) + (getAnchor1Y() * t * t), (getControl1X() * u) + (getAnchor1X() * t), (getControl1Y() * u) + (getAnchor1Y() * t), getAnchor1X(), getAnchor1Y()));
    }
    ...
    public static final class Companion {
        ...
        public final Cubic circularArc(float centerX, float centerY, float x0, float y0, float x1, float y1) {
            ...
            long rotatedP0 = Utils.m137rotate90DnnuFBc(p0d);
            long rotatedP1 = Utils.m137rotate90DnnuFBc(p1d);
            boolean clockwise = PointKt.m111dotProduct5P9i7ZU(rotatedP0, x1 - centerX, y1 - centerY) >= 0.0f;
            float cosa = PointKt.m112dotProductybeJwSQ(p0d, p1d);
            if (cosa > 0.999f) {
                return straightLine(x0, y0, x1, y1);
            }
            float k = ((((Utils.distance(x0 - centerX, y0 - centerY) * 4.0f) / 3.0f) * (((float) Math.sqrt(2 * (r1 - cosa))) - ((float) Math.sqrt(r1 - (cosa * cosa))))) / (1 - cosa)) * (clockwise ? 1.0f : -1.0f);
            return CubicKt.Cubic(x0, y0, x0 + (PointKt.m116getXDnnuFBc(rotatedP0) * k), y0 + (PointKt.m117getYDnnuFBc(rotatedP0) * k), x1 - (PointKt.m116getXDnnuFBc(rotatedP1) * k), y1 - (PointKt.m117getYDnnuFBc(rotatedP1) * k), x1, y1);
        }
    }
}
