    ...
    class Measurer implements BasicMeasure.Measurer {
        ...
        */
        public final void measure(ConstraintWidget widget, BasicMeasure.Measure measure) {
            ...
            int verticalDimension2 = measure.verticalDimension;
            int horizontalSpec2 = 0;
            int heightPadding = this.mPaddingTop + this.mPaddingBottom;
            int widthPadding = this.mPaddingWidth;
            ...
            switch (horizontalBehavior) {
                ...
                case WRAP_CONTENT:
                    verticalSpec = 0;
                    horizontalSpec2 = ViewGroup.getChildMeasureSpec(this.mLayoutWidthSpec, widthPadding, -2);
                    break;
                case MATCH_PARENT:
                    verticalSpec = 0;
                    horizontalSpec2 = ViewGroup.getChildMeasureSpec(this.mLayoutWidthSpec, widget.getHorizontalMargin() + widthPadding, -1);
                    break;
                case MATCH_CONSTRAINT:
                    int horizontalSpec3 = ViewGroup.getChildMeasureSpec(this.mLayoutWidthSpec, widthPadding, -2);
                    boolean shouldDoWrap = widget.mMatchConstraintDefaultWidth == 1;
                    int i2 = measure.measureStrategy;
                    int horizontalSpec4 = BasicMeasure.Measure.TRY_GIVEN_DIMENSIONS;
                    if (i2 != horizontalSpec4 && measure.measureStrategy != BasicMeasure.Measure.USE_GIVEN_DIMENSIONS) {
                        verticalSpec = 0;
                    } else {
                        boolean otherDimensionStable = child.getMeasuredHeight() == widget.getHeight();
                        int i3 = measure.measureStrategy;
                        verticalSpec = 0;
                        int verticalSpec4 = BasicMeasure.Measure.USE_GIVEN_DIMENSIONS;
                        boolean useCurrent = i3 == verticalSpec4 || !shouldDoWrap || (shouldDoWrap && otherDimensionStable) || (child instanceof Placeholder) || widget.isResolvedHorizontally();
                        if (useCurrent) {
            ...
            switch (verticalBehavior) {
                case FIXED:
                    verticalSpec2 = View.MeasureSpec.makeMeasureSpec(verticalDimension2, 1073741824);
                    break;
                case WRAP_CONTENT:
                    verticalSpec2 = ViewGroup.getChildMeasureSpec(this.mLayoutHeightSpec, heightPadding, -2);
                    break;
                case MATCH_PARENT:
                    verticalSpec2 = ViewGroup.getChildMeasureSpec(this.mLayoutHeightSpec, widget.getVerticalMargin() + heightPadding, -1);
                    break;
                case MATCH_CONSTRAINT:
                    int verticalSpec5 = ViewGroup.getChildMeasureSpec(this.mLayoutHeightSpec, heightPadding, -2);
                    boolean shouldDoWrap2 = widget.mMatchConstraintDefaultHeight == 1;
                    int i4 = measure.measureStrategy;
                    int horizontalDimension2 = BasicMeasure.Measure.TRY_GIVEN_DIMENSIONS;
                    if (i4 == horizontalDimension2 || measure.measureStrategy == BasicMeasure.Measure.USE_GIVEN_DIMENSIONS) {
                        boolean otherDimensionStable2 = child.getMeasuredWidth() == widget.getWidth();
                        int i5 = measure.measureStrategy;
                        verticalSpec3 = verticalSpec5;
                        int verticalSpec6 = BasicMeasure.Measure.USE_GIVEN_DIMENSIONS;
                        boolean useCurrent2 = i5 == verticalSpec6 || !shouldDoWrap2 || (shouldDoWrap2 && otherDimensionStable2) || (child instanceof Placeholder) || widget.isResolvedVertically();
                        if (useCurrent2) {
                            verticalSpec2 = View.MeasureSpec.makeMeasureSpec(widget.getHeight(), 1073741824);
                            break;
                        }
                    } else {
                        verticalSpec3 = verticalSpec5;
                    }
                    verticalSpec2 = verticalSpec3;
                    break;
                default:
            ...
            if (container != null && Optimizer.enabled(ConstraintLayout.this.mOptimizationLevel, 256) && child.getMeasuredWidth() == widget.getWidth() && child.getMeasuredWidth() < container.getWidth() && child.getMeasuredHeight() == widget.getHeight() && child.getMeasuredHeight() < container.getHeight() && child.getBaseline() == widget.getBaselineDistance() && !widget.isMeasureRequested()) {
                boolean similar = isSimilarSpec(widget.getLastHorizontalMeasureSpec(), horizontalSpec2, widget.getWidth()) && isSimilarSpec(widget.getLastVerticalMeasureSpec(), verticalSpec2, widget.getHeight());
                if (similar) {
            ...
            if (horizontalMatchConstraints && widget.mDimensionRatio > 0.0f) {
                ...
                if (verticalMatchConstraints && widget.mDimensionRatio > 0.0f) {
                    ...
                    long startMeasure2 = startMeasure;
                    if (measure.measureStrategy != BasicMeasure.Measure.TRY_GIVEN_DIMENSIONS && measure.measureStrategy != BasicMeasure.Measure.USE_GIVEN_DIMENSIONS && horizontalMatchConstraints && widget.mMatchConstraintDefaultWidth == 0 && verticalMatchConstraints && widget.mMatchConstraintDefaultHeight == 0) {
                        ...
                        if ((child instanceof VirtualLayout) && (widget instanceof androidx.constraintlayout.core.widgets.VirtualLayout)) {
                            ...
                            ((VirtualLayout) child).onMeasure(layout, horizontalSpec2, verticalSpec2);
                        } else {
                            child.measure(horizontalSpec2, verticalSpec2);
                        }
                        widget.setLastMeasureSpec(horizontalSpec2, verticalSpec2);
                        int w2 = child.getMeasuredWidth();
                        ...
                        int horizontalSpec5 = horizontalSpec2;
                        ...
                        if (!optimizeDirect) {
                            ...
                            } else if (verticalDimension != 0 && horizontalDimensionKnown) {
                                float ratio2 = widget.mDimensionRatio;
                                height = (int) ((width / ratio2) + 0.5f);
                            }
                        }
                        if (w2 == width && h == height) {
                            height2 = height;
                            w = baseline;
                        } else {
                            if (w2 == width) {
                                i = 1073741824;
                                horizontalSpec = horizontalSpec5;
                            } else {
                                i = 1073741824;
                    ...
                    boolean hasBaseline = w != -1;
                    measure.measuredNeedsSolverPass = (width == measure.horizontalDimension && height2 == measure.verticalDimension) ? false : true;
                    if (params.mNeedsBaseline) {
                        hasBaseline = true;
                    }
                    if (hasBaseline && w != -1 && widget.getBaselineDistance() != w) {
                        measure.measuredNeedsSolverPass = true;
                    }
                    measure.measuredWidth = width;
                    measure.measuredHeight = height2;
                    measure.measuredHasBaseline = hasBaseline;
                    measure.measuredBaseline = w;
                    if (ConstraintLayout.this.mMetrics != null) {
                        long endMeasure = System.nanoTime();
                        ConstraintLayout.this.mMetrics.measuresWidgetsDuration += endMeasure - startMeasure2;
                        return;
                    }
                    return;
                }
