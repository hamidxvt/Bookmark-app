...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initScreen(int parentWidth, int parentHeight, MotionKeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        if (!Float.isNaN(c.mPercentY)) {
            float pathVectorX2 = parentHeight - ((int) this.mHeight);
            this.mY = (int) (c.mPercentY * pathVectorX2);
        }
    ...
    void setView(float position, MotionWidget view, int[] toUse, double[] data, double[] slope, double[] cycle) {
        ...
        for (int i2 = 0; i2 < this.mTempValue.length; i2++) {
            boolean isNaN = Double.isNaN(this.mTempValue[i2]);
            double deltaCycle = Utils.DOUBLE_EPSILON;
            if (isNaN) {
                if (cycle == null) {
                    dv_height = dv_height2;
                    delta_path = delta_path2;
                } else if (cycle[i2] == Utils.DOUBLE_EPSILON) {
                    dv_height = dv_height2;
                    delta_path = delta_path2;
                }
        ...
        if (this.mRelativeToController != null) {
            ...
            float pos_y2 = (float) ((ry - (radius * Math.cos(angle))) - (v_height2 / 2.0f));
            ...
            if (!Float.isNaN(path_rotate2)) {
                v_y = pos_y2;
                dv_y = dpos_y;
                float rot = (float) (path_rotate2 + Math.toDegrees(Math.atan2(dpos_y, dpos_x)));
                motionWidget = view;
            ...
            if (!Float.isNaN(path_rotate3)) {
                ...
                float rot2 = 0.0f + ((float) (path_rotate3 + Math.toDegrees(Math.atan2(dy, dx))));
