...
public abstract class ConstraintController<T> implements ConstraintListener<T> {
    ...
    public final void setCallback(OnConstraintUpdatedCallback value) {
        if (this.callback != value) {
            this.callback = value;
            updateCallback(value, this.currentValue);
        }
    }
    ...
    public final void replace(Iterable<WorkSpec> workSpecs) {
        ...
        Collection destination$iv = this.matchingWorkSpecs;
        for (WorkSpec workSpec : workSpecs) {
            WorkSpec it = workSpec;
            if (hasConstraint(it)) {
                destination$iv.add(workSpec);
            }
        }
        Iterable destination$iv2 = this.matchingWorkSpecs;
        Iterable $this$mapTo$iv = destination$iv2;
        Collection destination$iv3 = this.matchingWorkSpecIds;
        for (Object item$iv : $this$mapTo$iv) {
            WorkSpec it2 = (WorkSpec) item$iv;
            destination$iv3.add(it2.id);
        }
