...
public final class WorkConstraintsTrackerImpl implements WorkConstraintsTracker, ConstraintController.OnConstraintUpdatedCallback {
    ...
    public WorkConstraintsTrackerImpl(WorkConstraintsCallback callback, ConstraintController<?>[] constraintControllers) {
    ...
    public void replace(Iterable<WorkSpec> workSpecs) {
        ...
        synchronized (this.lock) {
            ...
            for (ConstraintController controller2 : this.constraintControllers) {
                controller2.replace(workSpecs);
            }
            for (ConstraintController controller3 : this.constraintControllers) {
    ...
    public final boolean areAllConstraintsMet(String workSpecId) {
        ...
        synchronized (this.lock) {
            ...
            ConstraintController controller = constraintController;
            if (controller != null) {
                Logger logger = Logger.get();
                str = WorkConstraintsTrackerKt.TAG;
                logger.debug(str, "Work " + workSpecId + " constrained by " + controller.getClass().getSimpleName());
            }
            z = controller == null;
        }
        return z;
    ...
    public void onConstraintMet(List<WorkSpec> workSpecs) {
        ...
        Intrinsics.checkNotNullParameter(workSpecs, "workSpecs");
        synchronized (this.lock) {
            List<WorkSpec> $this$filter$iv = workSpecs;
            Collection destination$iv$iv = new ArrayList();
            for (Object element$iv$iv : $this$filter$iv) {
                WorkSpec it = (WorkSpec) element$iv$iv;
                if (areAllConstraintsMet(it.id)) {
                    destination$iv$iv.add(element$iv$iv);
                }
            }
            List unconstrainedWorkSpecIds = (List) destination$iv$iv;
            List $this$forEach$iv = unconstrainedWorkSpecIds;
            for (Object element$iv : $this$forEach$iv) {
                WorkSpec it2 = (WorkSpec) element$iv;
                Logger logger = Logger.get();
                str = WorkConstraintsTrackerKt.TAG;
                logger.debug(str, "Constraints met for " + it2);
            }
